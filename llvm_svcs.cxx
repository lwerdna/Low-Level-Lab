...
)
{
	...
	std::unique_ptr<MCRegisterInfo> regInfo(TheTarget->createMCRegInfo(machSpec));
	std::unique_ptr<MCAsmInfo> asmInfo(TheTarget->createMCAsmInfo(*regInfo, machSpec));
	std::unique_ptr<MCInstrInfo> instrInfo(TheTarget->createMCInstrInfo()); /* describes target instruction set */
	std::unique_ptr<MCSubtargetInfo> subTargetInfo(TheTarget->createMCSubtargetInfo(machSpec, "", "")); /* subtarget instr set */
	std::unique_ptr<MCAsmBackend> asmBackend(TheTarget->createMCAsmBackend(*regInfo, machSpec, /* specific CPU */ ""));
	MCInstPrinter *instrPrinter =  TheTarget->createMCInstPrinter(Triple(machSpec), /*variant*/0, *asmInfo, *instrInfo, *regInfo);
	...
	MCContext Ctx(asmInfo.get(), regInfo.get(), &MOFI, &SrcMgr);
	MOFI.InitMCObjectFileInfo(
		TheTriple, 
		Reloc::PIC_, // see also Reloc::Static, Reloc::PIC_, etc.
		CodeModel::Default,
		Ctx
	);
	...
	raw_string_ostream rso(outText);
	...
	MCCodeEmitter *codeEmitter = TheTarget->createMCCodeEmitter(*instrInfo, *regInfo, Ctx);

	MCStreamer *asmStreamer = TheTarget->createAsmStreamer(
		Ctx, /* the MC context */
	...
	/* create a vanilla AsmParser (lib/MC/MCParser/AsmParser.cpp) */
	std::unique_ptr<MCAsmParser> asmParser(
		llvm::createMCAsmParser(SrcMgr, Ctx, *asmStreamer, *asmInfo)
	);
	...
	switch(dialect) {
		...
		case LLVM_SVCS_DIALECT_ATT:
			asmParser->setAssemblerDialect(DIALECT_ATT);
			break;
		case LLVM_SVCS_DIALECT_INTEL:
			asmParser->setAssemblerDialect(DIALECT_INTEL);
			break;
	}
	...
	/* create a *TARGET* asm parser 
		(a target independent parser implementation to use for parsing and lexing)
		(include/llvm/Support/TargetRegistry.h) */
	std::unique_ptr<MCTargetAsmParser> targetAsmParser(
		TheTarget->createMCAsmParser(*subTargetInfo, *asmParser, *instrInfo, targetOpts)
	);
	if (!targetAsmParser) {
	...
	// now print hex dump
	//unsigned char *tmp = (unsigned char *)outBytes.data();
	//dump_bytes(tmp, outBytes.length(), 0);
